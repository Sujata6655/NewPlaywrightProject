...
```ts
  ...
  7   | 
  8   |   const currentPassword = '4e5r6t7y$E%R^T&i';
  9   |   const newPassword = '4e5r6t7y$E%R^T&s';
  10  |    const confirmPassword = '4e5r6t7y$E%R^T&s';
  11  | 
  12  | 
  13  |   // Open URL
  14  |   await page.goto(
...
> 28  |   await expect(firstName).toBeVisible({
  ...
  121 | 
  122 |  const guidelines = page.getByText('Password Guidelines');
  123 | 
  124 |  
  125 | 
  126 | await guidelines.waitFor({ state: 'visible' });
  127 | 
  128 | await guidelines.click();
```