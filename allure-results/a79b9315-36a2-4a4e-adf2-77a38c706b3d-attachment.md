...
```ts
  ...
  134 |   
  135 | 
  136 |     // Address validation popup
  137 |     const popupVisible = await this.useaddressbutton
  138 |       .isVisible()
  139 |       .catch(() => false);
  140 | 
  141 |     if (popupVisible) {
  142 |       await this.useaddressbutton.click();
  143 |       console.log('Address validation popup handled');
  144 |     } else {
  145 |       console.log('No address validation popup displayed');
  146 |     }
  147 | 
  148 |     // Wait for save completion
  149 |     console.log('Profile saved successfully');
  150 |   }
  151 |   
  152 |   async logout() {
  153 |     console.log('Starting Logout');
  154 | 
  155 |     // Close success toast/banner if present
  156 |     const successMessage = this.page.getByText(
  157 |       /success|updated|saved/i
  158 |     );
  159 | 
  160 |     if (await successMessage.isVisible().catch(() => false)) {
  161 |       await this.page.keyboard.press('Escape');
  162 |       await this.page.waitForTimeout(2000);
  163 |     }
  164 | 
  165 |     await this.profileDropdown.scrollIntoViewIfNeeded();
  166 | 
  167 |     await expect(this.profileDropdown).toBeVisible({
  168 |       timeout: 15000,
  169 |     });
  170 | 
  171 |     await this.profileDropdown.click({ force: true });
  172 | 
  173 | await this.page.waitForTimeout(2000);
  174 | 
  175 | const menuItems = await this.page.locator('li').allTextContents();
  176 | console.log('Menu Items:', menuItems);
  177 | 
  178 |     await this.page.waitForTimeout(2000);
  179 | 
> 180 |     await expect(this.logoutButton).toBeVisible({
      |                                     ^ Error: expect(locator).toBeVisible() failed
  181 |       timeout: 30000,
  182 |     });
  183 | 
  184 |     await this.logoutButton.click();
  185 | 
  186 |     console.log('Logout completed successfully');
  187 |   }
  188 | }
```