...
Call log:
  ...
  - attempting select option action
    ...
    - retrying select option action
      - waiting 100ms
    294 × waiting for element to be visible and enabled
        - did not find some options
      - retrying select option action
        - waiting 500ms
      ...
      - generic [ref=f4e36]:
        ...
        - generic [ref=f4e48]:
          ...
          - generic [ref=f4e54]:
            ...
            - textbox [active] [invalid] [ref=f4e58]
          ...
          - generic [ref=f4e79]:
            ...
            - textbox [ref=f4e83]
          - generic [ref=f4e84]:
            - button "Cancel" [ref=f4e85] [cursor=pointer]
            - button "Save" [ref=f4e86] [cursor=pointer]
      - separator [ref=f4e87]
      - alert [ref=f4e88]:
        - img "Info:" [ref=f4e90]
        - generic [ref=f4e92]: We highly recommend you verify your phone number. Once verified, you can recover your User Account using SMS text as identity verification. You will also have the added benefit of receiving SMS text communications from John Deere.Verify your phone number.
      - generic [ref=f4e98]:
        - generic [ref=f4e99]:
          - paragraph [ref=f4e100]: Phone Number
          - img [ref=f4e101] [cursor=pointer]
        - generic [ref=f4e105]:
          - textbox "1 (702) 123-4567" [disabled] [ref=f4e106]: "+919876543229"
          - 'button "India: + 91" [ref=f4e108]'
      - separator [ref=f4e111]
    - generic [ref=f4e116]:
      - paragraph [ref=f4e117]: Email Address *
      - textbox [disabled] [ref=f4e121]: sunil.yadav@yopmail.com
      - generic [ref=f4e124]: Edit or change of email address is not allowed if Username is an email address.
    - separator [ref=f4e125]
    - button "Back to previous page" [ref=f4e126] [cursor=pointer]
  - generic [ref=f4e127]:
    - contentinfo [ref=f4e128]:
      - navigation "footer navigation" [ref=f4e129]:
        - list [ref=f4e130]:
          - listitem [ref=f4e131]:
            - link "Privacy and Data" [ref=f4e132] [cursor=pointer]:
              - /url: https://www.deere.com/en/privacy-and-data/
          - listitem [ref=f4e133]:
            - link "Terms of Use" [ref=f4e134] [cursor=pointer]:
              - /url: https://www.deere.com/en/privacy-and-data/myjohndeere/terms/
          - listitem [ref=f4e135]:
            - link "Contact Us" [ref=f4e136] [cursor=pointer]:
              - /url: https://account-helppages.deere.com/contact-us
      - generic [ref=f4e137]: Copyright © 2026 Deere & Company. All Rights Reserved.
    - list
```
...
> 70  | await this.state.selectOption({ label: state });
      |                  ^ Error: locator.selectOption: Test timeout of 180000ms exceeded.
  71  | 
  72  | // Wait for form refresh/API call after state selection
  73  | await this.page.waitForLoadState('networkidle');
  74  | 
  75  | await this.address.scrollIntoViewIfNeeded();
  76  | await expect(this.address).toBeVisible();
  77  | await this.address.click();
  78  | await this.address.fill(address);
  79  | 
  80  | // If address autocomplete is present
  81  | await this.page.keyboard.press('ArrowDown');
  82  | await this.page.keyboard.press('Enter');
  83  | 
  84  | await expect(this.address).toHaveValue(address, { timeout: 10000 });
  85  | 
  86  | await this.city.scrollIntoViewIfNeeded();
  87  | await expect(this.city).toBeVisible();
  88  | await this.city.fill(city);
  89  | 
  90  | await expect(this.city).toHaveValue(city, { timeout: 10000 });
  91  | 
  92  | await this.zipCode.scrollIntoViewIfNeeded();
  93  | await expect(this.zipCode).toBeVisible();
  94  | await this.zipCode.fill(zipCode);
  95  | 
  96  | await expect(this.zipCode).toHaveValue(zipCode, { timeout: 10000 });
  97  | 
  98  | console.log('Address:', await this.address.inputValue());
  99  | console.log('City:', await this.city.inputValue());
  100 | console.log('Zip:', await this.zipCode.inputValue());
  101 | 
  102 | await this.savePersonalInfoButton.click();
  103 |  
  104 | }
  105 |   
  106 |     
  107 |   
  108 | 
  109 |   async openChangePassword() {
  110 |     await expect(this.changePasswordButton).toBeVisible({
  111 |       timeout: 30000,
  112 |     });
  113 | 
  114 |     await this.changePasswordButton.scrollIntoViewIfNeeded();
  115 |     await this.changePasswordButton.click();
  116 |   }
  117 | 
  118 |   async logout() {
  119 |     await expect(this.profileDropdown).toBeVisible();
  120 | 
  121 |     await this.profileDropdown.click();
  122 | 
  123 |     await expect(this.logoutButton).toBeVisible();
  124 | 
  125 |     await this.logoutButton.click();
  126 |   }
  127 | }
```