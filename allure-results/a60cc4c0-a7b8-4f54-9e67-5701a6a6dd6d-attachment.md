...
Call log:
  ...
  - element was detached from the DOM, retrying
    - locator resolved to <button disabled tabindex="-1" type="submit" class="MuiButtonBase-root MuiButton-root MuiButton-secondary MuiButton-secondaryPrimary MuiButton-sizeSmall MuiButton-secondarySizeSmall MuiButton-colorPrimary Mui-disabled MuiButton-root MuiButton-secondary MuiButton-secondaryPrimary MuiButton-sizeSmall MuiButton-secondarySizeSmall MuiButton-colorPrimary css-z1moln">Save</button>
  - attempting click action
    2 × waiting for element to be visible, enabled and stable
      - element is not enabled
    - retrying click action
...
```ts
  ...
  131 | 
  132 | ).fill(newPassword);
  133 | await page.keyboard.press('Tab');
  134 | 
  135 | 
  136 | 
  137 |   // Save Password
> 138 |   await page.getByRole('button', { name: /save/i }).click();
      |                                                     ^ Error: locator.click: Test timeout of 120000ms exceeded.
  139 | 
  140 |   // Wait for Success Message
  141 |   await page.waitForTimeout(5000);
  142 | 
  143 |   // Open Profile Dropdown
  144 |   const profileDropdown = page.locator(
  145 |     '/html/body/div/div/div/div/div/header/div/div/div/button'
  146 |   );
  147 | 
  148 |   await expect(profileDropdown).toBeVisible({
  149 |     timeout: 30000,
  150 |   });
  151 | 
  152 |   await profileDropdown.click();
  153 | 
  154 |   // Logout
  155 |   const logoutButton = page.locator(
  156 |     '/html/body/div[2]/div[3]/ul/li'
  157 |   );
  158 | 
  159 |   await expect(logoutButton).toBeVisible({
  160 |     timeout: 10000,
  161 |   });
  162 | 
  163 |   await logoutButton.click();
  164 | 
  165 |   // Open URL Again
  166 |   await page.goto(
  167 |     'https://account.deere.com/actmgmt/profile?TARGET=https://map.deere.com/'
  168 |   );
  169 | 
  170 |   // Login With New Password
  171 |   await page.locator('#input28').fill(email);
  172 |   await page.locator('#form20 input[type="submit"]').click();
  173 | 
  174 |   await page.locator('#input54').fill(newPassword);
  175 |   await page.locator('#form46 input[type="submit"]').click();
  176 | 
  177 |   // Verify Successful Login
  178 |   await expect(
  179 |     page.locator('input[name="firstName"]')
  180 |   ).toBeVisible({
  181 |     timeout: 60000,
  182 |   });
  183 | 
  184 |   console.log('Successfully logged in with new password');
  185 | });
  186 | 
```