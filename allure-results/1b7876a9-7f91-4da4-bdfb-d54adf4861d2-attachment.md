...
Locator: locator('xpath=/html/body/div[2]/div[3]/ul/li')
Expected: visible
Error: element(s) not found

Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for locator('xpath=/html/body/div[2]/div[3]/ul/li')
  - Test timeout of 30000ms exceeded.
...
```ts
  ...
  135 |   
  136 | async logout() {
  137 |   const successMessage = this.page.getByText(/success|updated/i);
  138 | 
  139 |   if (await successMessage.isVisible().catch(() => false)) {
  140 |     await successMessage.waitFor({
  141 |       state: 'hidden',
  142 |       timeout: 10000
  143 |     });
  144 |   }
  145 | 
  146 |   await this.profileDropdown.scrollIntoViewIfNeeded();
  147 | 
  148 |   await expect(this.profileDropdown).toBeVisible({
  149 |     timeout: 10000
  150 |   });
  151 | 
  152 |   await this.profileDropdown.click({ force: true });
  153 | 
> 154 |   await expect(this.logoutButton).toBeVisible({
      |                                   ^ Error: expect(locator).toBeVisible() failed
  155 |     timeout: 10000
  156 |   });
  157 | 
  158 |   await this.logoutButton.click();
  159 | }
  160 | }
  161 | 
```