...
```ts
  ...
  19 | 
  20 |  const [editProfilePage] = await Promise.all([
> 21 |     context.waitForEvent('page'),page.locator("#form46 input[type='submit']").click()
     |             ^ Error: browserContext.waitForEvent: Test timeout of 30000ms exceeded.
  22 |  ]);
  23 |  
  24 |  //await expect(page).toHaveTitle('Edit Profile');
  25 |  const nameField = page.locator('//*[@id=":r2:"]');
  26 | 
  27 | // Check field is visible and enabled
  28 | 
  29 | await expect(nameField).toBeVisible();
  30 | 
  31 | await expect(nameField).toBeEnabled();
  32 | const frame = page.frameLocator('iframe');
  33 | 
  34 | // Clear existing value
  35 | 
  36 | await nameField.clear();
  37 | 
  38 | // Enter new name
  39 | await nameField.fill('Sunil S');
  40 |  await usernameInput.waitFor({ state: 'visible' });
  41 | //click on save button
  42 | await page.locator('//*[@id="addressDetailsId"]/div[5]/button[2]').click();
  43 | // Verify success message is displayed
  44 | 
  45 | await expect(page.getByText(/success/i)).toBeVisible();
  46 | 
  47 | });
  48 | 
```